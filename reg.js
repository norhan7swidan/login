...
function login() {
    ...
    .then(data => {
        ...
        if (data.success && !successMessageDisplayed) { // تحقق من عدم إظهار الرسالة من قبل
            successMessageDisplayed = true; // قم بتعيين المتغير للتأكيد على إظهار الرسالة مرة واحدة
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    emailInput.addEventListener('input', function () {
        ...
        if (email !== '') {
            ...
            const savedEmails = localStorage.getItem('savedEmails') ? JSON.parse(localStorage.getItem('savedEmails')) : [];
